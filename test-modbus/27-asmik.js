...
const master = new modbus.ModbusMaster(new SerialPort(port, {
  ...
  // dataBits: 8, // default: 8. 8, 7, 6, 5
  // stopBits: 1, // default: 1. 1, 2
  // parity: 'none', // default: none. 'none', 'even', 'odd', 'mark', 'space'
}), {
  endPacketTimeout: 19,
  queueTimeout: 50,
  responseTimeout: 500
...
async function async_all_read() {
  var promises = _.map(regs, reg => {
    return master.readInputRegisters(addr, reg.addr, 2, float_be).catch(console.error)
    return master.readInputRegisters(addr, reg.addr, parsers[reg.format].length, parsers[reg.format].parse).then(({ value, hex }) => {
